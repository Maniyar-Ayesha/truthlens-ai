...
def main():
    if len(sys.argv) < 2:
        print("Usage: python train_video_classifier.py <dataset_directory>")
        sys.exit(1)

    dataset_dir = sys.argv[1]
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(script_dir, "frame_classifier.pt")
    ...
    if os.path.isdir(dataset_dir):
        real_dir = os.path.join(dataset_dir, "real")
        fake_dir = os.path.join(dataset_dir, "fake")
        has_real = os.path.isdir(real_dir) and len([
            f for f in os.listdir(real_dir)
            if f.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".tiff"))
        ]) > 0
        has_fake = os.path.isdir(fake_dir) and len([
        ...
        if has_real or has_fake:
            ...
            if len(full_dataset) > 0:
                ...
                n_val = int(math.floor(n_total * 0.2))
                n_train = n_total - n_val
