...
class SyntheticImageDataset(Dataset):
    def __init__(self, n_samples=2000, image_size=224, num_classes=2):
        self.samples = []
        random.seed(42)
        np.random.seed(42)
...
def train_model(model, train_dataset, val_dataset, device, epochs=10, batch_size=32):
...
def main():
    ...
    save_path = os.path.join(script_dir, "resnet_model.pt")
    metadata_path = os.path.join(script_dir, "resnet_metadata.json")
